...
namespace Render
{
    std::shared_ptr<void> PointLightJson::Parse(const json11::Json &j)
    {
        ...
        auto colorIntensity = colorJArr.size() > 3 ? (float)colorJArr.at(3).number_value() : 1.0f;
        ...
        pointLight->color = glm::vec4(colorR, colorG, colorB, colorIntensity);
    ...
    json11::Json PointLightJson::Serialize(std::shared_ptr<void> component)
    {
        ...
        auto colorJArr = json11::Json::array{color.r, color.g, color.b, color.a};
